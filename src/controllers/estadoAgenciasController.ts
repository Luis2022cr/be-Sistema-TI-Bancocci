...
export const crearEstadoAgencias = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        await pool.query('INSERT INTO estado_agencias (nombre) VALUES (?)', [nombre]);
...
export const actualizarEstadoAgencias = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const { nombre } = req.body;
        ...
        const [result]: any = await pool.query('UPDATE estado_agencias SET nombre = ? WHERE id = ?', [nombre, id]);
